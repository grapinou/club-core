...
func NewAuthHandler(siteName string, a Activator, login *auth.Service, sessions *auth.Sessions, secure bool) *AuthHandler {
	return &AuthHandler{siteName: siteName, activation: a, auth: login, sessions: sessions, secure: secure, limiter: NewAttemptLimiter(), crossOrigin: http.NewCrossOriginProtection()}
}
func (h *AuthHandler) Register(mux *http.ServeMux) {
	...
	mux.Handle("POST /logout", h.sensitive(h.postLogout))
}
func (h *AuthHandler) render(w http.ResponseWriter, r *http.Request, activation bool, message string) {
	...
	data := views.AuthData{SiteName: h.siteName, Title: title + " - " + h.siteName, Activation: activation, Message: message, CSRFToken: token, LoggedIn: loggedIn}

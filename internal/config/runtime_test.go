...
func TestRuntime(t *testing.T) {
	...
	for _, tc := range []struct{ key, value string }{
		...
		{"EMAIL_CHANGE_TTL", "0s"}, {"EMAIL_CHANGE_TTL", "-1h"}, {"EMAIL_CHANGE_TTL", "bad"}, {"REGISTRATION_VERIFICATION_TTL", "0s"}, {"REGISTRATION_VERIFICATION_TTL", "-1h"}, {"REGISTRATION_VERIFICATION_TTL", "bad"}, {"ACTIVATION_CODE_TTL", "0s"}, {"APP_TIMEZONE", "unknown"},
	} {
	...
	if err != nil || cfg.EmailChangeTTL != DefaultEmailChangeTTL || cfg.EmailTransport != "disabled" || cfg.SecureCookies || cfg.RegistrationVerificationTTL != DefaultRegistrationVerificationTTL {
